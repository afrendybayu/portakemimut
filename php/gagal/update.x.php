...
try {
	...
	$params->lok = '';
	$params->nama = '';
	$params->downt = '2014-01-17';
	$params->downj = '10:30' ;
	$params->upt = '2014-01-17';
	$params->upj = '20:30'; 
	$params->event = '2' ;		// 1: standby
	$params->idevent = ''; 
	$params->tipeev = '';
	$params->ket ='';
	$params->exe = '';
	...
	//$wkt = cek_waktu_range($idAr[0], $params->downt, $params->downj, $params->upt, $params->upj);		// old
	...
	echo "kombinasi waktu: ".count($kw)."<br/>"; print_r($kw);	echo "<br/><br/>";
	...
	$ar=array();	$ar_av=array();	$ar_re=array();
	...
	//echo "Running Hour: "; print_r($hrh); echo "<br/>";
	//echo "Availability: "; print_r($hrh_av); echo "<br/>";
	//echo "Reliability : "; print_r($hrh_re); echo "<br/>";
	

	$rh = format_rh($hrh);
	//echo "Format_rh RunnningHour: "; print_r($rh); echo "<br/>";
	$rh_av = format_rh($hrh_av);
	//echo "Format_rh Availability: "; print_r($rh_av); echo "<br/>";
	$rh_re = format_rh($hrh_re);
	//echo "Format_rh Reliability : "; print_r($rh_re); echo "<br/>";
	...
	if ($level=='u') {
		for($i=0; $i<$idJml; $i++)	{
				...
				if ($params->event==1) {		// standby
					$sql = "INSERT INTO waktudown (eqid,unit_id,downt,downj,upt,upj,event,ket,nginput) ".
							"VALUES ('{$idAr[$i]}','{$id}','{$params->downt}','{$params->downj}', '{$params->upt}', '{$params->upj}', ".
							"'{$params->event}','{$params->ket}','{$now}' )";
				} else {
					for($w=0; $w<$jmlpm; $w++)	{
						if ($idAr[$i]==$pm[$w][0])	break;
					}
					$sql = "INSERT INTO waktudown (eqid,unit_id,downt,downj,upt,upj,startt,startj,endt,endj,".
	...
	do {
		...
		if ($adatgl->jml==0)	{
			...
			$sql =	"INSERT INTO rh_201311 (eq,tgl,rh,rh_av, rh_re,flag) VALUES ".
