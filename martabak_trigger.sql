...
-- Dumping structure for trigger medco.ohlist_ADEL
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `ohlist_ADEL` AFTER DELETE ON `ohlist` FOR EACH ROW begin
	delete from overhaul where ohlist_id = old.id; 
end//
DELIMITER ;
	...
	while lm < l do 
		begin 
			while (m <= 4 and lm < l) do 
				begin 
					insert into overhaul (ohlist_id,equip, oh, pekan, bln, thn) values (new.id,new.equip,new.ohcat, m, b, t );
					set m = m+1; 
					set lm = lm+1;
				end;
			end while;
...
-- Dumping structure for trigger medco.ohlist_AUPD
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `ohlist_AUPD` AFTER UPDATE ON `ohlist` FOR EACH ROW begin
	declare l,b,t,w,m,lm int;
